#!/bin/bash
# 컨테이너 안에서 도는 GUI 세션 기동 스크립트 (U-IA-02·06).
#
# 순서: VNC 비밀번호 발급 → Xvnc 기동(빈 포트 확보) → connection.json 기록 → 앱 실행.
# 앱이 끝나면(로그아웃/종료) Xvnc를 정리하고 종료한다 = Slurm Job 종료.
#
# **이미지는 하나이고 앱은 `PORTAL_APP`으로 고른다.** 클러스터 설정의 이미지 참조가
# 필드 하나라, 앱마다 SIF를 나누면 운영이 복잡해진다.
set -euo pipefail
...
case "${PORTAL_APP:-desktop}" in
    desktop)
        ...
        ;;
    paraview)
        # 창 관리자 기동과 최소화 차단은 start-paraview.sh가 한다.
        # gsettings가 dbus 세션 안에서 돌아야 해서 한 덩어리로 묶었다.
        dbus-launch --exit-with-session /opt/portal/start-paraview.sh &
        APP_PID=$!
        ;;
    *)

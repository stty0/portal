...
분리하면 장애 영향 범위가 좁아진다. 포털은 홈을 SSSD/NSS(`getent passwd`)로 사용자별 자동
인식하므로 별도 등록 항목이 없다 — 홈 경로가 NSS와 일치하기만 하면 된다.
`/scratch`·`/groups` 같은 공유 볼륨은 **포털 범위 밖**이다(정의서 §4.1). 마운트해도
파일 관리자에는 나타나지 않으며, 사용자는 웹 터미널이나 직접 SSH로 접근한다.

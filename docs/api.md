...
| GET | `/apps` | 앱 목록 — **코드 카탈로그의 앱 + 등록된 메타데이터를 합친다**(아이콘·벤더·버전·`image_file`·설명). `id=null`은 아직 정보가 등록되지 않은 코드 앱, `in_code=false`는 코드에 없는 등록(도입 예정). 실행 방식은 포함하지 않는다 | A-OP-02 | 인증 | 필수 |

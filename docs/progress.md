...
`test_settings_require_admin`이 "Authorization 없음 = 401"을 전제하고 있었는데 이제 비관리자로
인증돼 403이 됐다. 테스트에서 쿠키를 명시적으로 비우도록 고쳤다 — 앞으로 익명 상태를 보려면
`client.cookies.clear()`가 필요하다.

검증: 테스트 **313개 통과**(8개 추가). 비어 있지 않음 확인 — CSRF 검사를 빼자 1개, refresh
회전(폐기)을 빼자 1개가 각각 실패했고 복구하니 전부 통과했다.

...
### 정합성 감사 — 발견 6건 정정 (2026-08-10)

기계적 대조는 양호했다: 라우트↔`api.md` 완전 일치, 모델↔ERD **22개 표·컬럼까지 일치**,
계층 규칙 7개 통과, 제거된 기능의 코드 잔재 없음.

#### 🔴 보안 — 개인키가 `repr`에 평문으로 나온다

`SshTarget`이 평범한 `@dataclass`라 기본 `repr`이 **모든 필드를 찍었다.** 이건 가설이
아니다 — 오늘 진단 코드가 이 객체를 출력하면서 **192.168.1.201의 개인키가 세션 로그에
평문으로 남았다.**

로그로 새는 경로는 없었다(로그 5곳 모두 이 객체를 안 담고 `unhandled_error_handler`가
메시지를 숨긴다). 문제는 **다음에 누가 디버그 로그 한 줄을 넣으면 바로 샌다**는 것이다.
`field(repr=False)`로 구조적으로 막았다.

같은 계열로 `Settings`의 `jwt_secret`·`database_url`·`redis_url`·`setup_token`도
`repr`에서 뺐다(DB·Redis URL에는 비밀번호가 들어 있다). 값은 그대로 쓰이고 **표시만** 가린다.

테스트 2개로 고정했다 — 보호를 되돌리면 둘 다 실패한다. 실 클러스터 자격증명으로도
확인했다(`repr`에 키 없음, `private_key`는 정상 동작).

⚠️ **유출된 키는 교체해야 한다.** 코드를 고쳐도 이미 로그에 남은 값은 되돌릴 수 없다.

#### 🟠 진단 문구가 하지 말아야 할 일을 시켰다

`이미지 디렉터리가 없습니다 — 만들고 **관리자 그룹에 쓰기 권한을 주세요**`

변환 잡이 그 디렉터리에 써야 했을 때 만든 문구인데, T-08 철회로 **포털은 여기 쓰지
않는다.** 게다가 이 클러스터에는 관리자 그룹이 없어서 "그룹 쓰기"는 곧 `domain users`에
여는 것이고, 그건 **"컨테이너 이미지는 모두가 실행하는 코드"라서 하지 않기로 한 바로
그것**이다. 관리자가 문구대로 따랐으면 아무나 남이 실행할 이미지를 바꿀 수 있었다.

→ `이 경로를 만들고 SIF를 넣어 주세요. 포털은 이 디렉터리에 쓰지 않으므로 쓰기 권한은
필요하지 않습니다.` 로 바꿨다. 테스트가 **"쓰기 권한을 주라고 하지 않는다"**를 고정한다.

#### 그 밖

- `check()` docstring이 없어진 T-08을 근거로 들고 있었다 → 지금 사실로
- `AppImageCache.invalidate()` 호출자 없음(유일한 호출자가 `install()`이었다) → 제거.
  클래스 머리말도 "무효화 지점을 둘 자리가 애초에 없다"로 정정
- `plan.md`·`exec-plan.md`가 **철회된 T-07·T-08을 계획으로** 담고 체크박스 24개가 전부
  비어 있었다 → 완료/철회 표시. 다음 세션이 **이미 배포된 것을 다시 만들** 위험이 있었다
- `plan.md` 제목·§3 결정·§5 위험표에서 레지스트리 변환 전제 정리
- `exec-plan.md` T-05 수용 기준이 방금 고친 **잘못된 문구를 그대로** 담고 있었다 → 정정
- `gpu-simulation.md` §4: 목적지 경로가 옛 `/home/images`였다 → `{home_base}/.portal/images`.
  `docker://`로 데몬 없이도 된다는 실측과 변환 소요(9분 3초/2.4GB)도 함께 적었다
- `정의서.md` A-OP-02에 `0018`→`0019` 이력 한 줄 추가

검증: 테스트 **434개 통과**(2개 추가). 배포 후 실 클러스터 자격증명으로 `repr` 확인.

---

### 앱 관리 화면 로딩 — 원인과 정정

느리다는 지적을 받고 파드에서 구간별로 쟀다.

| 구간 | 시간 |
|---|---|
| `GET /apps` (DB) | 2.3 ms |
| 아이콘 디렉터리 스캔(NFS) | 4.0 ms |
| **`app-images` 클러스터당 (콜드)** | **653 ms** |
| 〃 캐시 적중 | 0.2 ms |

SSH가 99%였고, 그 안을 다시 쪼개면 **연결 115~154ms + 첫 `list_dir`(sudo sftp-server 스폰)
120~260ms + 실제 조회 23ms**다. **조회는 23ms이고 나머지는 전부 연결 수립 비용**이며,
그것을 클러스터 수만큼 낸다. 캐시가 60초라 1분마다 다시 낸다 — "가끔 느리다"의 정체다.

**진짜 문제는 그 목록을 페이지에서 쓰지 않는다는 것이었다.** `imageFiles`가 들어가는 곳은
모달 안 `<datalist>` 하나뿐인데, 페이지 로드에서 받고 있었다. 목록만 보고 나가는 대부분의
경우에 1.3초를 아무 이유 없이 냈다.

→ **모달을 열 때 받도록 미뤘다**(`ensureImageFiles`). 모달을 막지 않고(호출자가 기다리지
않는다) 제안 목록이 잠시 뒤 채워지며, 화면에 머무는 동안 한 번만 받는다. 실패해도 폼은
그대로 쓸 수 있다 — 파일명은 직접 입력된다.

페이지 로드가 실제로 하는 일이 **57ms**가 됐다(전: + SSH 2회 ≈ 1.3초).

`ls` exec으로 sftp-server 스폰을 없애는 안도 있었지만 택하지 않았다 — 출력 파싱이 생기고
`FileEntry`로 깔끔한 것을 흐린다. 지연 로딩만으로 체감 문제가 사라진다.

#### 번들 검증이 감사가 놓친 잔재를 잡았다

배포 후 **번들 안 문자열을 확인하다가** 파일명 칸 힌트가 아직 `"변환 결과가 저장될 이름"`
이라고 말하는 것을 발견했다 — 변환 기능은 이미 없앴는데 문구가 남아 있었다. 바로 위
주석도 `"변환 경로가 막힌다"`로 없어진 기능을 근거로 들고 있었다. 둘 다 정정했다.
...
### 이미지 목록에 확장자 필터 — `.sif`·`.sqsh`만

목록과 `installed` 판정이 `IMAGE_FILE` 정규식 하나로 걸러지고 있었는데, **그건 경로
안전성(`../` 차단)을 보는 것이지 형식 판별이 아니었다.** 그래서 이미지 디렉터리에 둔
`README.txt`·`notes.md`·`archive.tar.gz`가 **전부 선택지에 떴다.** 그걸 고르면
`installed=true`가 되고 잡은 워커에서 죽는다.

두 질문을 분리했다 — `IMAGE_FILE`은 경로 경계 그대로 두고, `is_image_name()`이 확장자를
함께 본다. 확장자는 **대소문자를 가리지 않는다**(`IMAGE.SIF`가 디렉터리에 있는데 목록에만
안 뜨면 원인을 찾기 어렵다). 이름 자체는 그대로 쓴다 — 리눅스는 대소문자를 구분한다.

`.sqsh`(enroot)를 지금부터 통과시킨다(사용자 결정). 나중에 한 번 더 고치지 않아도 된다.

⚠ **`.sqsh`는 목록에만 반영된다.** 실행은 아직 apptainer 하나뿐이라, 지금 `.sqsh`를
등록하면 `apptainer exec`로 돌다 실패한다. 런타임 분기(확장자 → apptainer/enroot, 자원은
`needs_gpu`가 따로)는 enroot가 클러스터에 깔릴 때 붙인다 — 코드 주석에 그 사실을 적었다.
지금 클러스터에 enroot·pyxis는 둘 다 없다(실측).

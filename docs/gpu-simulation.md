...
| 4 | 사용자 API 토큰 | 외부 자동화가 AD 비밀번호를 스크립트에 박아야 한다 | 남음 (HttpOnly 쿠키·refresh는 2026-08-08 완료) |
...
갱신해야 한다(refresh 2주, 회전). 그래서 4번(사용자별 API 토큰)이 여전히 필요하다.
사용자에 묶인 토큰이므로 "대상 사용자는 언제나 요청자 본인" 규칙과 충돌하지 않는다.
...
**3단계** — 2단계에서 나온 "매번 바꾸는 값"이 곧 입력 폼이다. 인터랙티브 앱
카탈로그(`session_apps.py`)와 같은 모양으로 solver 정의를 코드에 두고 이미지·파라미터 스키마를
붙인다.

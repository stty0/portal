...
- **BaseClient ↔ 개별 Client(AdClient·SlurmClient)**: `is-a`(상속) — 공통 로직(재시도·타임아웃·에러 매핑) 재사용.
...
- **1단계(현재)**: 단순 Role 기반. role-permission 매핑을 **코드 상수(dict)** 로.
- **2단계(확장)**: Permission을 `resource:action`(`job:read`, `job:cancel`)으로 명명, DB 테이블화(`Role`, `Permission`, `RolePermission` N:M).
...
```python
# 로그인 시 세션 등록
r.setex(f"session:{username}", timedelta(hours=8), token)

# 요청마다 JWT 서명 검증 + 세션 존재 확인
if not r.exists(f"session:{payload['sub']}"):
...
- **SSH/SFTP**: 파일 관리(U-FM)·웹 터미널(U-SH)·인터랙티브 세션(U-IA) → `clients/ssh`(서비스 계정 + 키, 제한 sudo impersonation).

...
class AppImageService:
    ...
    def check(self, cluster: Cluster, *, username: str) -> dict[str, object]:
        ...
        **포털은 이 디렉터리에 쓰지 않는다.** 만들지도 않고 파일을 넣지도 않는다 —
        `{home_base}`는 root 소유라 만들려면 권한 상승이 필요하고, 그건 "허용 루트는
        홈뿐"이라는 경계를 깨는 첫 사례가 된다. 이미지는 관리자가 포털 밖에서 받아
        SIF로 바꾼 뒤 직접 올려놓는다.
        """
        directory = image_dir(cluster, self.settings)
        try:
            ...
            return {
                ...
                "ok": False,
                "images": 0,
                # **"그룹에 쓰기 권한을 주라"고 하면 안 된다.** 이 클러스터에는 관리자
                # 그룹이 없어서 그 말은 곧 `domain users`에 여는 것이고, 그러면 아무나
                # *남이 실행할 이미지*를 바꿔 놓을 수 있다. 포털은 여기 쓰지 않으므로
                # 열어 줄 이유도 없다.
                "message": (
                    f"이미지 디렉터리가 없습니다: {directory} — "
                    "클러스터에서 이 경로를 만들고 SIF를 넣어 주세요. "
                    "포털은 이 디렉터리에 쓰지 않으므로 쓰기 권한은 필요하지 않습니다."
                ),
            }
        except Exception:
